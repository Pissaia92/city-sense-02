...
const AppContent = () => {
  ...
  const [showComparisonSuggestions, setShowComparisonSuggestions] =
    useState(false);
  ...
  const [comparisonForecast, setComparisonForecast] = useState<
    ForecastPoint[] | null
  >(null);
  ...
  const fetchSuggestions = async (
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/iqv?city=${encodeURIComponent(query)}`
      );
      if (response.ok) {
  ...
  const fetchData = async (cityName: string) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        setError(
          '🔌 Connection error. Please check if the backend is running at https://dashboard.render.com/web/srv-d2ee2fidbo4c738b8dd0'
        );
      } else {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !mainSearchRef.current.contains(event.target as Node)
      )
        setShowMainSuggestions(false);
      if (
        comparisonSearchRef.current &&
        !comparisonSearchRef.current.contains(event.target as Node)
      )
        setShowComparisonSuggestions(false);
    };
  ...
  const formatTrafficDelay = (delay: number) =>
    delay <= 0 ? '0 minutes' : `${Math.round(delay)} minutes`;
        ...
        >
              ...
              <div>
                <CityHeader
                  ...
                  getWeatherIcon={() => {
                    ...
                    if (desc.includes('chuva') || desc.includes('storm'))
                      return '⛈️';
                ...
                <MetricsGrid
                  data={data}
                  formatTrafficDelay={formatTrafficDelay}
                  darkMode={darkMode}
                />
              </div>
              <div>
              ...
              <div style={{ padding: '16px' }}>
                {mlPrediction ? (
                  ...
                  >
                    ...
                    >
                      Processing QoL prediction with machine learning...
                    </p>
                  </div>
                )}
              </div>
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          handleComparisonInputChange(e.target.value)
                        }
                ...
                {comparisonLoading ? (
                  ...
                  <div style={{ marginTop: '16px' }}>
                    <CityComparison
                      cities={
                        data
                          ? [data.city, comparisonCity]
                          : ['São Paulo', comparisonCity]
                      }
                      darkMode={darkMode}
                      shouldFetch={true}
                    />
          ...
          {data && forecast && forecast.length > 0 && (
            ...
            >
              ...
              >
                ...
                >
                  Expected Temperatures - {data.city}
                </h2>
              </div>
              <div style={{ padding: '16px' }}>
            ...
            comparisonForecast.length > 0 && (
              ...
              >
                ...
                >
                  ...
                  >
                    Expected Temperatures - {comparisonCity}
                  </h2>
                </div>
                <div style={{ padding: '16px' }}>
                  <ForecastChart
            ...
            comparisonForecast !== null && (
              ...
              >
                ...
                <div style={{ padding: '16px', textAlign: 'center' }}>
                  ...
                  >
                    Weather forecast not available for {comparisonCity}
                  </p>
                </div>
              </div>
            )}
          ...
          {!data && searchTried && !loading && !error && (
            ...
            >
              ...
              >
                Type the name of a city in the field above to see its urban
                quality of life
              </p>
            </div>
          )}
