...
@router.get("/api/predict/QoL")
@limiter.limit("10/minute")
async def predict_QoL(
    request: Request,
    city: str = Query(..., description="City name", example="Brasília")
):
    """Predict future QoL for a city (placeholder implementation)"""
    # try:
    ...
    #         base_QoL = 75.0 - (i * 2)  # Gradually decreasing
    #         variation = (i % 3) * 5  # Small variations
            
    #         prediction = {
    #             "date": date.strftime("%Y-%m-%d"),
    #             "predicted_QoL": max(0, min(100, base_QoL + variation)),
